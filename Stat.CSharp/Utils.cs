...
namespace StatsMasterDemo
{
    class Utils
    {
        ...
        private static string StatsMasterRegKey = @"Software\Dew Research\Dew Stats";
        private static string NETDemoKey = ".NET Demo";
        ...
        public static string ReadDemoPath()
        {
            ...
            if (key != null)
            {
                RegistryKey rootPath = key.CreateSubKey(NETDemoKey);
                if (rootPath != null)
                {
                    object o = rootPath.GetValue("Source Folder");
        ...
        public static void WriteDemoPath(string path)
        {
            RegistryKey key = Registry.CurrentUser.CreateSubKey(StatsMasterRegKey);
            if (key != null)
            {
                RegistryKey rootPath = key.CreateSubKey(NETDemoKey);
                rootPath.SetValue("Source Folder", path);
            }
        }

        #endregion

        #region Demo Tree Node

        internal class DemoTreeNode : TreeNode
        {
